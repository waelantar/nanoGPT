"""
Light benchmark script to compare Wide & Deep embeddings vs standard embeddings
"""
import time
import torch
...
def benchmark_embeddings():
    ...
    # Parameters (lighter for memory constraints)
    vocab_size = 2000
    embedding_dim = 256
    batch_size = 8
    seq_len = 64
    ...
    print(f"- Time for 5 forward passes: {wide_deep_time:.4f}s")
    print(f"- Average time per forward pass: {wide_deep_time/5*1000:.2f}ms")
    
    print()
    
    print(f"Standard Embedding:")
    print(f"- Parameters: {std_params:,}")
    print(f"- Time for 5 forward passes: {standard_time:.4f}s")
    print(f"- Average time per forward pass: {standard_time/5*1000:.2f}ms")
    
    print()
    
    print(f"Comparison:")
    print(f"- Wide & Deep is {((wide_deep_time/5) / (standard_time/5) - 1) * 100:+.1f}% {'slower' if wide_deep_time > standard_time else 'faster'} than standard")
...
if __name__ == "__main__":
    print("Running Light Wide & Deep Embedding Benchmarks\n")
